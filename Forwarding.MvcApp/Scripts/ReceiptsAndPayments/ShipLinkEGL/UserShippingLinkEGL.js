function UserShippingLinkEGL_BindTableRows(pUserShippingLink) {
    debugger;
...
function UserShippingLink_LoadingWithPaging() {
    ...
    LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", strLoadWithPagingFunctionName, pWhereClause, pOrderBy, pPageNumber, pPageSize, pControllerParameters, function (pData) { UserShippingLinkEGL_BindTableRows(JSON.parse(pData[0])); });
    HighlightText("#tblUserShippingLink>tbody>tr", $("#txt-Search").val().trim());
}
function UserShippingLinkEGL_GetWhereClause() {
...
function UserShippingLink_Insert(pSaveandAddNew) {
    debugger;
    InsertUpdateFunction("form", "/api/UserShippingLinkEGL/Insert", { pUserID: $("#slUsers").val(), pUserDasID: $("#slUserDas").val() }, pSaveandAddNew, "UserShippingLinkModal", function () { UserShippingLink_LoadingWithPaging(); });
}

function UserShippingLink_Update(pSaveandAddNew) {
    InsertUpdateFunction("form", "/api/UserShippingLinkEGL/Update", { pID: $("#hID").val(), pUserID: $("#slUsers").val(), pUserDasID: $("#slUserDas").val() }, pSaveandAddNew, "UserShippingLinkModal", function ()
...
function UserShippingLinkEGL_DeleteList() {
    ...
    if (GetAllSelectedIDsAsString('tblUserShippingLink', 'Delete') != "")
        ...
        function () {
            DeleteListFunction("/api/UserShippingLinkEGL/Delete", { "pIDs": GetAllSelectedIDsAsString('tblUserShippingLink', 'Delete') }, function () {

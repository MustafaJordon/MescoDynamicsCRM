...
function YardInvoicePosting_BindTableRows(pTableRows) {
    debugger;
    $("#hl-menu-ShipLink").parent().addClass("active");
    ClearAllTableRows("tblYardInvoicePosting");
    //editControlsText = " class='btn btn-xs btn-rounded btn-info float-right' > <i class='fa fa-pencil' style='padding-left: 5px;'></i> <span style='padding-right: 5px;'>" + TranslateString("Edit") + "</span>";
    $.each(pTableRows, function (i, item) {
        AppendRowtoTable("tblYardInvoicePosting",
    ...
    //ApplyPermissions();
    BindAllCheckboxonTable("tblYardInvoicePosting", "ID");
    CheckAllCheckbox("ID");
...
function YardInvoicePosting_LoadingWithPaging() {
    debugger;
    ...
    LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", strLoadWithPagingFunctionName, pWhereClause, pOrderBy, pPageNumber, pPageSize, pControllerParameters, function (pData) { YardInvoicePosting_BindTableRows(JSON.parse(pData[0])); });
    //HighlightText("#tblYardInvoicePosting>tbody>tr", $("#txt-Search").val().trim());
}
function YardInvoicePosting_GetWhereClause() {
    debugger;
  ...
  //  pWhereClause += " where JVID IS NULL" + "\n";
    ...
    if (isValidDate($("#txtSearchFrom").val().trim(), 1) && $("#txtSearchFrom").val().trim() != "")
        pWhereClause += " AND CONVERT(date,IssueDate,103) >= '" + GetDateWithFormatyyyyMMdd($("#txtFromDate").val().trim()) + "'" + " \n";
    if (isValidDate($("#txtSearchTo").val().trim(), 1) && $("#txtSearchTo").val().trim() != "")
        pWhereClause += " AND CONVERT(date,IssueDate,103) <= '" + GetDateWithFormatyyyyMMdd($("#txtToDate").val().trim()) + "'" + " \n";
...
function YardInvoicePosting_LinkRevenueItems() {
    debugger;
    var pSelectedIDs = GetAllSelectedIDsAsString('tblYardInvoicePosting', 'Delete');
    if (pSelectedIDs != "") {
        FadePageCover(true);
        CallGETFunctionWithParameters("/api/YardInvoicePosting/GetRevenueItemsModalData"
            , {
...
function YardInvoicePosting_Post(pIsSaveRevenueItems) {
    debugger;
    var isValid = true;
    var pPostedSelectedIDs = GetAllSelectedIDsAsString('tblYardInvoicePosting', 'Delete');
    if (pPostedSelectedIDs != "") {
        ...
        if (isValid) {
            ...
            CallGETFunctionWithParameters("/api/YardInvoicePosting/Post"
                , {
                ...
                , function (pData) {
                    ...
                    YardInvoicePosting_LoadingWithPaging();

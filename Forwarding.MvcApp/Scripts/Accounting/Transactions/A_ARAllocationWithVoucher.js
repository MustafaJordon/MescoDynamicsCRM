function  A_ARAllocationWithVoucher_Initialize()
{
    debugger;
    ...
    CallGETFunctionWithParameters("/api/A_ARAllocationWithVoucher/ARAllocationWithVoucher_Partners", { pIsLoadArrayOfObjects: true }
    , function (pData) {
...
function ARAllocation_PartnerTypeChanged() {
    ...
    if ($("#slPartnerType").val() != "") {
        FadePageCover(true);
        CallGETFunctionWithParameters("/api/A_ARAllocationWithVoucher/FillPartners", { pPartnerTypeID: $("#slPartnerType").val() }
            , function (pData) {
...
function A_ARAllocationWithVoucher_BindTableRows(pJVTypes) {
    debugger;
    ...
    var pParametersWithValues =
        {
           ...
           , pTitle: "Liner Allocation Cash Collection Receipt"
            , pReportName: "Rep_A_Payment_Allocation"
        };
...
function Allocation_Insert(pSaveandAddNew) {
    ...
    if (result[0] == result2[0] && Total.toFixed(2) != Total2.toFixed(2))
    {
    ...
    CallPOSTFunctionWithParameters("/api/A_ARAllocationWithVoucher/Save", pParametersWithValues
       , function (pData) {
           if (pData[0] == '') {
...
function Allocation_DeleteList() {
    ...
    if (GetAllSelectedIDsAsString('tblPayments', 'Delete') != "")
        ...
        function () {
            ...
            FadePageCover(true);
            CallGETFunctionWithParameters("/api/A_ARAllocationWithVoucher/Delete"
            ...
            //DeleteListFunction("/api/A_ARAllocationWithVoucher/Delete", { "pPaymentIDs": GetAllSelectedIDsAsString('tblPayments', 'Delete') }, function () {
            //    Allocation_LoadingWithPaging();
            //});
        });
}
...
function loadWithPagingWithParmWhere(pWhereClause, pWhereClauseVoucher) {
    debugger;
    LoadWithPagingWithWhereClause("div-Pager1", "select-page-size1", "spn-first-page-row1", "spn-last-page-row1", "spn-total-count1", "div-Text-Total1", "/api/A_ARAllocationWithVoucher/LoadInvoices", pWhereClause, 1, 99999, function (pTableRows) { PaymentsAllocationInvoices_BindTableRows(pTableRows); });
    LoadWithPagingWithWhereClause("div-Pager1", "select-page-size1", "spn-first-page-row1", "spn-last-page-row1", "spn-total-count1", "div-Text-Total1", "/api/A_ARAllocationWithVoucher/LoadVoucher", pWhereClauseVoucher, 1, 99999, function (pTableRows) {
        PaymentsA_Voucher_BindTableRows(JSON.parse(pTableRows[0]));
...
function  LoadData()
{
    ...
    if ($("#slClient option:selected").text() != "" && $("#slClient option:selected").val() != ""
        ...
        var pWhereClause = " Where PartnerID = '" + ($("#slClient option:selected").val() == "" ? "" : $("#slClient option:selected").val()) + "'"
          ...
          var pWhereClauseVoucher = " Where Rmaining > 0 and Client_ID = '" + ($("#slClient option:selected").val() == "" ? "" : $("#slClient option:selected").val()) + "' and PartnerTypeID= " + ($("#slPartnerType option:selected").val() == "" ? "" : $("#slPartnerType option:selected").val());
...
function InvoiceSearch() {
    ...
    if ($("#slClient option:selected").text() != "" && $("#slClient option:selected").val() != "0") {
        ...
        pWhereClause += " and PartnerTypeID = '" + ($("#slPartnerType option:selected").val() == "" ? "" : $("#slPartnerType option:selected").val()) + "'";
    ...
    if (pWhereClause != "") {
        LoadWithPagingWithWhereClause("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/A_ARAllocationWithVoucher/LoadInvoices", pWhereClause, 1, 99999, function (pTableRows) { PaymentsAllocationInvoices_BindTableRows(pTableRows); });
...
function VoucherSearch() {
    if ($("#slClient option:selected").text() != "" && $("#slClient option:selected").val() != "0") {
        ...
        LoadWithPagingWithWhereClause("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/A_ARAllocationWithVoucher/LoadVoucher", pWhereClause, 1, 99999, function (pTableRows)
        {
            PaymentsA_Voucher_BindTableRows(JSON.parse(pTableRows[0]));          
        });
...
function PaymentsAllocationInvoices_BindTableRows(pInvoices) {
    ClearAllTableRows("tblInvoices");
    $("#txtTotalPaid").val("");
    $('#lblTotalAllocation').html('');
    TotalCost = 0;
    ...
    $.each(pInvoices, function (i, item) {
        ...
        ("<tr  style = 'display:block; ' Invoiceid='" + item.ID + "' class='" + ((item.IsDeleted == true) ? "deleted-row" : "") + "'>"
                    ...
                    + "<td class='Code'>" + item.InvoiceNumber + "</td>"
                    + "<td class='InvoiceTypeName'>" + item.InvoiceTypeName + "</td>"
                    + "<td class='Date'>" + ConvertDateFormat(GetDateWithFormatMDY(item.InvoiceDate)) + "</td>"
                   ...
                   // + "<td class='DebitCreditNotes_NO text-overflow-150 hide' title='" + item.DebitCreditNotes_NO + "'>" + item.DebitCreditNotes_NO + "</td>"
                    ...
                    + "<td class='InvoiceTotal align-right' style=''>" + item.CurrencyCode + " " + item.Amount + "</td>"
                    ...
                    + "<td class='Choose'> <input id='txtChoose"+ item.ID  +"' class='input_Choose' type='checkbox'  onchange='SelectRowInTable(" + i + ");'/></td>"
                    + "</tr>"));
...
function PaymentsA_Voucher_BindTableRows(pInvoices) {
    ClearAllTableRows("tblA_Voucher");
    $("#txtTotalVoucherPaid").val("");
    $('#lblTotalAllocationVoucher').html('');
    ...
    $.each(pInvoices, function (i, item) {
        ...
        ("<tr ID='" + item.ID + "' class='" + ((item.IsDeleted == true) ? "deleted-row" : "") + "'>"
                  ...
                  //  + "<td class='ExchangeRate' style=''>" + item.ExchangeRate + "</td>"
                    ...
                    + "<td class='AmountTextCurrency' hidden >" + item.CurrencyCode + "</td>"
                    + "<td class='Choose'> <input id='txtChoose" + item.ID + "' type='checkbox'   class='input_Choose' onchange='SelectRowInVoucher(" + i + ");'/></td>"
                    + "</tr>"));
        // if ($("#slCurrency").val() != "-1" && $("#slCurrency").val() != 0 && $("#slCurrency").val() != null) {
...
function CalculateAll() {
    ...
    //$('#txtTotalDiff').val(TotalVoucherPaid.toFixed(2) - TotalPaid.toFixed(2)
    //    - ($('#txtCurrencyProfit').val() == '' ? 0 : parseFloat($('#txtCurrencyProfit').val()))
    //    + ($('#txtCurrencyLoss').val() == '' ? 0 : parseFloat($('#txtCurrencyLoss').val())));

    $('#txtTotalDiff').val((TotalVoucherPaid.toFixed(2) - TotalPaid.toFixed(2)
    - ($('#txtCurrencyProfit').val() == '' ? 0 : parseFloat($('#txtCurrencyProfit').val()))
    + ($('#txtCurrencyLoss').val() == '' ? 0 : parseFloat($('#txtCurrencyLoss').val()))).toFixed(2));
...
function Allocation_LoadingWithPaging() {
    ...
    var pPageNumber = 1;
    var strLoadWithPagingFunctionName = "/api/A_ARAllocationWithVoucher/LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned";
    var pOrderBy = "ID DESC";
    var pControllerParameters = { pIsLoadArrayOfObjects: false, pLanguage: $("[id$='hf_ChangeLanguage']").val(), pPageNumber: pPageNumber, pPageSize: pPageSize, pWhereClause: pWhereClause, pOrderBy: pOrderBy }
    LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", strLoadWithPagingFunctionName, pWhereClause, pOrderBy, pPageNumber, pPageSize, pControllerParameters, function (pData) { A_ARAllocationWithVoucher_BindTableRows(JSON.parse(pData[0])); });
    HighlightText("#tblPayments>tbody>tr", $("#txt-Search").val().trim());
}
function AllocationInvoice_GetWhereClause() {
    return ($("#txt-Search").val().trim() == "" ? "WHERE 1=1" : ("WHERE Code ='" + $("#txt-Search").val().trim() + "'"

                                                          + " OR ClientName LIKE N'%" + $("#txt-Search").val().trim() + "%' "));
}

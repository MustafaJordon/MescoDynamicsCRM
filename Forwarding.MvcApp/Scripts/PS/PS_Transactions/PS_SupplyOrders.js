...
function PrintSupplyOrders(SupplyOrderID) {
    FadePageCover(true);
    if (SupplyOrderID == 0)
        SupplyOrderID = $('#hID').val();
    $('#hExportedTable').html('');
    LoadAll("/api/PS_SupplyOrders/LoadDetails", "where ID = " + SupplyOrderID, function (data) {
        var pReportTitle = " Supply Order - أمر توريد ";
        ...
        $(_Details).each(function (i, item) {
            ...
            if ($(_Details).length - 1 == i) {
                ...
                _InvDate = GetDateFromServer(item.PurchasingSupplyDate);
                _InvCode = item.PurchasingSupplyNo;
                ...
                _PurchasingOrderInfo = IsNull( item.PurchasingOrderInfo , "--");
                _PurchasingQuotationInfo = IsNull( item.QuotationNo , "") + ' - ' + IsNull(item.QuotationNoManual , "") + ' - ' + IsNull( GetDateFromServer( item.QuotationDate) , "")  ;
                _PaymentTermName = item.PaymentTermName;
                _PaymentNotes = item.PaymentNotes;
                pTablesHTML += '<tr>';
        ...
        ReportHTML += '                 <div class="col-xs-3"><b>' + (lang == "ar" ? "  رقم أمر التوريد " : "  SupplyOrder No ") + ': </b> ' + _InvCode + '</div>';
        ReportHTML += '                 <div class="col-xs-3"><b>' + (lang == "ar" ? "  تاريخ أمر التوريد " : "  Date ") + ': </b> ' + _InvDate + '</div>';

        ReportHTML += '                 <div class="col-xs-3"><b>' + (lang == "ar" ? "  بند الدفع " : "  Payment Term ") + ': </b> ' + _PaymentTermName + '</div>';
        ReportHTML += '                 <div class="col-xs-3"><b>' + (lang == "ar" ? "  شورط الدفع " : "  Payment Details ") + ': </b> ' + _PaymentNotes + '</div>';


        ReportHTML += '                 <div class="col-xs-3"><b>' + (lang == "ar" ? "  أمر شراء  " : "  Purchasing Order ") + ': </b> ' + _PurchasingOrderInfo + '</div>';
        ReportHTML += '                 <div class="col-xs-3"><b>' + (lang == "ar" ? "  عرض سعر  " : "  Quotation No ") + ': </b> ' + _PurchasingQuotationInfo + '</div>';
...
function PrintQuotation() {
    debugger
    var QuotationID = IsNull( $('#slPurchasingQuotation').val() , "0");
    FadePageCover(true);
    if (QuotationID == 0)
        QuotationID = $('#hID').val();
    $('#hExportedTable').html('');
...
function PrintPurchasingOrders() {
    debugger
    var PurchasingOrderID = IsNull($('#slPurchasingOrders').val(), "0");
    FadePageCover(true);
...
function IntializePage() {
    debugger;
    var WhereClause = " Where ISNULL(vwPS_SupplyOrders.IsDeleted , 0) = 0";
    LoadWithPagingWithWhereClause("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/PS_SupplyOrders/LoadWithWhereClause", WhereClause, $("#div-Pager li.active a").text(), $('#select-page-size').val().trim(), function (pTabelRows) { PS_SupplyOrders_BindTableRows(pTabelRows); });
    $.ajax({
        type: "GET",
        url: strServerURL + "/api/PS_SupplyOrders/IntializeData",
        ...
        success: function (d) {
            ...
            Fill_SelectInputAfterLoadData_WithMultiAttrAndMultiFields(d[8], 'ID', 'QuotationNo,QuotationNoManual', '-', 'Select Quotation No - اختر عرض السعر', '#slPurchasingQuotation', '', 'QuotationNo,SupplierID,CurrencyID');
            Fill_SelectInputAfterLoadData_WithMultiAttrAndMultiFields(d[9], 'ID', 'PurchasingOrderNo,PurchasingOrderNoManual', '-', 'Select Purchasing Order - اختر أمر الشراء', '#slPurchasingOrders', '', 'PurchasingOrderNo,SupplierID,CurrencyID');

            Fill_SelectInputAfterLoadData_WithMultiAttrAndMultiFields(d[10], 'ID', 'Code,Name', '-', 'Select Payment Term - اختر بند الدفع', '#slPaymentTerms', '', 'Code,Name,Days');

            $('#txtSupplyOrderDate').val(getTodaysDateInddMMyyyyFormat());

            $("#txtSupplyOrderDate").datepicker().on('changeDate'
                , function () {
                    $(this).datepicker('hide');
                    // RecalculateExchangeRate();

                });
            $("#txtSupplyOrderDate").datepicker().on('keydown', function (ev) { if (ev.keyCode == 9) $(this).datepicker('hide'); });
...
function PS_HideShowEditBtns(IsApproved, HasTransactions) {
    ...
    if ($('#IsFromQuotations').is(':checked'))
    {
        ...
        $('#slPurchasingOrders').prop('disabled', true);

        $('.IsPurchasingQuotationsArea').removeClass('hide');
        $('.IsPurchasingOrdersArea').addClass('hide');
        $('.IsManualArea').addClass('hide');

        $('#slPurchasingOrders').val("0");
    }
    else if ($('#IsFromPurchasingOrders').is(':checked')) {
        $('#slSupplierID').prop('disabled', true);
        $('#slCurrencyID').prop('disabled', true);
        ...
        $('#slPurchasingQuotation').prop('disabled', true);
        $('#slPurchasingOrders').prop('disabled', false);

        $('.IsPurchasingQuotationsArea').addClass('hide');
        $('.IsPurchasingOrdersArea').removeClass('hide');
        $('.IsManualArea').addClass('hide');
...
function RecalculateExchangeRate() {
    ...
    $.ajax({
        type: "GET",
        url: strServerURL + "/api/PS_SupplyOrders/IntializeData",
        data: { pDate: ConvertDateFormat($('#txtSupplyOrderDate').val()), pID: $('#hID').val() },
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (d) {
...
function IntializeData(callback) {
    ...
    $.ajax({
        ...
        success: function (d) {
            ...
            Fill_SelectInputAfterLoadData_WithMultiAttrAndMultiFields(d[2], 'ID', 'PurchasingOrderNo,PurchasingOrderNoManual', '-', 'Select Purchasing Order - اختر أمر الشراء', '#slPurchasingOrders', '', 'PurchasingOrderNo,SupplierID,CurrencyID');
...
function SelectSupplierAndCurrencyFromPurchasingOrder() {

    var SelectedPurchasingOrder = $('#slPurchasingOrders option:selected');

    if (IsNull(SelectedPurchasingOrder, null) != null) {

        $('#slSupplierID').val($('#slPurchasingOrders option:selected').attr('supplierid'));
        $('#slCurrencyID').val($('#slPurchasingOrders option:selected').attr('currencyid'));
...
function PS_SupplyOrders_ClearAllControls()
{
    ...
    ClearAll("#PS_SupplyOrdersModal", null);
    $('#hID').val("0");
    $('#slSupplierID').val("0");
    $('#slCurrencyID').val("0");
    $('#slPurchasingQuotation').val("0");
    $('#slPurchasingOrders').val("0");
    $("#btnSave").attr("onclick", "PS_SupplyOrders_Save(false);");
    $("#btnSaveandNew").attr("onclick", "PS_SupplyOrders_Save(true);");
    $("#cb-CheckAll").prop('checked', false);
    $('#txtSupplyOrderNo').val("Auto");
    $('#txtSupplyOrderNoManual').prop("disabled", false);
...
function LoadDetails() {
    ...
    LoadAll("/api/PS_SupplyOrders/LoadDetails", "where ID = " + $('#hID').val(), function (pTabelRows) {
        PS_SupplyOrdersDetails_BindTableRows(pTabelRows[0]);
...
function LoadPurchasingOrdersDetails(THIS_PurchasingOrders) {
    debugger;

    LoadAll("/api/PS_PurchasingOrders/LoadDetails", "where ID = " + $(THIS_PurchasingOrders).val(), function (pTabelRows) {
...
//#region Header
function PS_SupplyOrders_BindTableRows(pPS_SupplyOrders)
{
    debugger;
    $("#hl-menu-PS").parent().addClass("active");
    ClearAllTableRows("tblPS_SupplyOrders");
    $.each(pPS_SupplyOrders, function (i, item) {
        debugger;
        var disable = "";
        ...
        AppendRowtoTable("tblPS_SupplyOrders",
            ("<tr ID='" + item.ID + "' ondblclick='PS_SupplyOrders_EditByDblClick(" + item.ID + " , " + item.IsApproved + "); '>"
                ...
                + "<td class='hPS_SupplyOrders hide'><a href='#PS_SupplyOrdersModal' data-toggle='modal' onclick='PS_SupplyOrders_FillControls(" + item.ID + ");' " + editControlsText + "</a></td>"
                + "<td class='pPS_SupplyOrders'><a href='#' onclick='PrintSupplyOrders(" + item.ID + ");' " + printControlsText + "</a></td></tr > "));
    });
    ApplyPermissions();
    BindAllCheckboxonTable("tblPS_SupplyOrders", "ID");
    CheckAllCheckbox("ID");
    HighlightText("#tblPS_SupplyOrders>tbody>tr", $("#txt-Search").val().trim());
...
function PS_SupplyOrders_EditByDblClick(pID, pIsApproved, pHasTransactions) {
    _IsApproved = pIsApproved;
    _HasTransactions = pHasTransactions;
    $('#btnPrint2').removeClass('hide');
    jQuery("#PS_SupplyOrdersModal").modal("show");
    ...
    PS_SupplyOrders_FillControls(pID);
}

function PS_SupplyOrders_LoadingWithPaging() {
    debugger;
    var WhereClause = " Where ISNULL(vwPS_SupplyOrders.IsDeleted , 0) = 0 ";

    if ($('#txtSupplyOrderNo_Filter').val().trim() != "") {
        WhereClause += " AND SupplyOrderNo = '" + $('#txtSupplyOrderNo_Filter').val() + "'";
    }
    if ($('#txtFromDate_Filter').val().trim() != "") {
        WhereClause += " AND CONVERT(date , SupplyOrderDate ) >= CONVERT(date , '" + ConvertDateFormat($('#txtFromDate_Filter').val()) + "')";
    }
    if ($('#txtToDate_Filter').val().trim() != "") {
        WhereClause += " AND CONVERT(date , SupplyOrderDate) <=  CONVERT(date ,'" + ConvertDateFormat($('#txtToDate_Filter').val()) + "')";
    }

    LoadWithPagingWithWhereClause("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/PS_SupplyOrders/LoadWithWhereClause", WhereClause, $("#div-Pager li.active a").text(), $('#select-page-size').val().trim(), function (pTabelRows) { PS_SupplyOrders_BindTableRows(pTabelRows); PS_SupplyOrders_ClearAllControls(); });
    HighlightText("#tblPS_SupplyOrders>tbody>tr", $("#txt-Search").val().trim());
}
...
function PS_SupplyOrders_DeleteList() {
    ...
    if (GetAllSelectedIDsAsString('tblPS_SupplyOrders') != "") {
        swal({
            title: "Are you sure?",
            ...
            function () {
                DeleteListFunction("/api/PS_SupplyOrders/Delete", { "pPS_SupplyOrdersIDs": GetAllSelectedIDsAsString('tblPS_SupplyOrders') }, function () { PS_SupplyOrders_LoadingWithPaging(); });
            });
    }
    //DeleteListFunction("/api/PS_SupplyOrders/Delete", { "pPS_SupplyOrdersIDs": GetAllSelectedIDsAsString('tblPS_SupplyOrders') }, function () { PS_SupplyOrders_LoadingWithPaging(); });
}
//after pressing edit, this function fills the data
function PS_SupplyOrders_FillControls(pID) {
    debugger;
    ClearAll("#PS_SupplyOrdersModal", null);
    $('#btnPrint2').removeClass('hide');
    ...
    IntializeData(function () {
        setTimeout(function () {
            var tr = $("#tblPS_SupplyOrders > tbody > tr[ID='" + pID + "']");
            FadePageCover(true)
            ...
            $("#slPaymentTerms").val($(tr).find("td.PaymentTermID").attr('val'));
            $("#txtPaymentTermNotes").val($(tr).find("td.PaymentNotes").attr('val'));
            $("#txtSupplyOrderDate").val($(tr).find("td.PurchasingSupplyDate").attr('val'));
            $("#slPurchasingQuotation").val($(tr).find("td.PS_QuotationsID").attr('val'));
            $("#slPurchasingOrders").val($(tr).find("td.PS_PurchasingOrdersID").attr('val'));
            $("#txtSupplyOrderNo").val($(tr).find("td.PurchasingSupplyNo").attr('val'));
            $("#txtSupplyOrderNoManual").val($(tr).find("td.PurchasingSupplyNoManual").attr('val'));
            $("#txtNotes").val($(tr).find("td.Notes").attr('val'));


            if (IsNull($(tr).find("td.PS_QuotationsID").attr('val'), "0") != "0") {
                $('#IsFromQuotations').prop('checked', true);
                $('#IsFromManual').prop('checked', false);
                $('#IsFromPurchasingOrders').prop('checked', false);

            }
            else if (IsNull($(tr).find("td.PS_PurchasingOrdersID").attr('val'), "0") != "0") {
                $('#IsFromQuotations').prop('checked', false);
                $('#IsFromManual').prop('checked', false);
                $('#IsFromPurchasingOrders').prop('checked', true);
            ...
            else
            {
                ...
                $('#IsFromQuotations').prop('checked', false);
                $('#IsFromManual').prop('checked', true);
                $('#IsFromPurchasingOrders').prop('checked', false);
            }
...
function GetInsertUpdateParameters() {
    debugger
    var SupplyOrderNo = (($('#txtSupplyOrderNo').val() == "" || $('#txtSupplyOrderNo').val() == "Auto") ? "0" : $('#txtSupplyOrderNo').val());
    var SupplyOrderNoManual = (($('#txtSupplyOrderNoManual').val() == "" || $('#txtSupplyOrderNoManual').val() == "Auto") ? "0" : $('#txtSupplyOrderNoManual').val());
    var SupplyOrderDate = ConvertDateFormat($('#txtSupplyOrderDate').val());
    ...
    var PurchasingOrdersID = IsNull($('#slPurchasingOrders').val(), "0");
    var PaymentTermID = IsNull($('#slPaymentTerms').val(), "0");
    var PaymentTermNote = IsNull($('#txtPaymentTermNotes').val(), "0");


    return {
        "pID": ID,
        "pPurchasingSupplyNo": SupplyOrderNo,
        "pPurchasingSupplyDate": SupplyOrderDate,
        "pNotes": Notes,
        "pPS_QuotationsID": PurchasingQuotationID, 
        "pPS_PurchasingOrdersID": PurchasingOrdersID,
        "pPaymentTermID": PaymentTermID,
        "pPaymentNotes": PaymentTermNote,
        ...
        "pPurchasingSupplyNoManual": SupplyOrderNoManual,
...
function PS_SupplyOrders_Save(pSaveandAddNew) {
    IsInsert = true;
    var _Suceess = true;
    ...
    if (IsNull($('#slPurchasingOrders').val(), "0") == "0" && $('#IsFromPurchasingOrders').is(':checked')) {
        swal((lang == "ar" ? 'معذرة' : 'Excuse me'), (lang == "ar" ? 'من فضلك اختر أمر الشراء ' : 'You must Select Purchasing Orders'), 'warning');
        FadePageCover(false);
        _Suceess = false;
    }
    ...
    if (_Suceess == true) {
        ...
        setTimeout(function () {
            debugger
            FadePageCover(true)
            PostInsertUpdateFunction("form", "/api/PS_SupplyOrders/Save", GetInsertUpdateParameters(), pSaveandAddNew, null,
                ...
                function (res) {
                    ...
                    InsertUpdateListOfObject("/api/PS_SupplyOrders/InsertItems",
                        ListOfListOfObject
                        , pSaveandAddNew, "PS_SupplyOrdersModal", function (data) {
                            FadePageCover(true)
                            setTimeout(function () {
                                PrintSupplyOrders(res[1]);
                                PS_SupplyOrders_LoadingWithPaging();
                            }, 300);
                ...
                if (data != undefined && data.length > 1) {
                    if (data != 0) {
                        ...
                        if (callback != null && callback != undefined) {
                            if (data[1] != null && data[1] != undefined) //data[1] is the ID: for opening SupplyOrder or operation after saving a new one / or strMessageReturned
                                callback(data);
                        }
...
function PS_SupplyOrders_Delete() {
    swal({
        title: "Are you sure?",
        ...
        //callback function in case of success
        function () {
            InsertUpdateFunction("form", "/api/PS_SupplyOrders/Delete",
                { 'pTransactionsID': $('#hID').val(), 'pTransactionDate': ConvertDateFormat($('#txtDate').val()) + " 07:00:00 PM" }
                , false, "PS_SupplyOrdersModal", function (data) {
                    if (data[1].trim() == '') {
                        PS_SupplyOrders_LoadingWithPaging();
                        //IntializeData();
...
function PS_SupplyOrdersDetails_DeleteList() {
    debugger;
    //Confirmation message to delete
    if (GetAllSelectedIDsAsString('tblItems') != "")
        swal({
            ...
            //callback function in case of success
            function () {
                DeleteListFunction("/api/PS_SupplyOrders/Delete", { "pPS_SupplyOrdersDetailsIDs": GetAllSelectedIDsAsString('tblPS_SupplyOrdersDetails') }, function () { PS_SupplyOrdersDetails_LoadingWithPaging(); });
            });
}
...
function PS_SupplyOrdersDetails_BindTableRows(pItems) {
    debugger;
    $("#hl-menu-PS").parent().addClass("active");
    ItemsRowsCounter = 0;
    ClearAllTableRows("tblItems");
...
function PS_PurchasingOrdersDetails_BindTableRows(pItems) {
    ...
    SelectSupplierAndCurrencyFromPurchasingOrder();
...
function SC_PurchaseItems_LoadAll() {
    debugger;
    LoadAll("/api/PS_SupplyOrders/LoadItems", "where PurchaseInvoiceID = " + $('#slPSInvoices').val(), function (pTabelRows) { SC_PurchaseItems_BindTableRows(pTabelRows) });
    // LoadWithPaging("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/PS_SupplyOrders/LoadWithPaging", $("#div-Pager li.active a").text(), $('#select-page-size').val().trim(), function (pTabelRows) { PS_SupplyOrders_BindTableRows(pTabelRows); PS_SupplyOrders_ClearAllControls(); });
    // HighlightText("#tblPS_SupplyOrders>tbody>tr", $("#txt-Search").val().trim());
}
...
function SetArrayOfItems() {
    ...
    $("#tblItems>tbody>tr").each(function (i, tr) {
        ...
        objItem.PS_SupplyOrdersID = $('#hID').val();

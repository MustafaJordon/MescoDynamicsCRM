...
function PS_ApprovePurchasingOrders_BindTableRows(pPS_Invoices) {
    debugger;
    $("#hl-menu-PS").parent().addClass("active");
    ClearAllTableRows("tblPS_ApprovePurchasingOrders");
    $.each(pPS_Invoices, function (i, item) {
        debugger;

        //editControlsText = " class='btn btn-xs btn-rounded btn-info float-right' > <i class='fa fa-pencil' style='padding-left: 5px;'></i> <span style='padding-right: 5px;'>" + TranslateString("Edit") + "</span>";
        AppendRowtoTable("tblPS_ApprovePurchasingOrders",
    ...
    ApplyPermissions();
    BindAllCheckboxonTable("tblPS_ApprovePurchasingOrders", "ID");
    CheckAllCheckbox("ID");
    HighlightText("#tblPS_ApprovePurchasingOrders>tbody>tr", $("#txt-Search").val().trim());
...
function PS_ApprovePurchasingOrders_LoadingWithPaging() {
    debugger;
...
function PS_ApprovePurchasingOrders_Approve() {

    console.log(GetAllSelectedIDsAsString("tblPS_ApprovePurchasingOrders"));
    var pSelectedIDs = GetAllSelectedIDsAsString("tblPS_ApprovePurchasingOrders");
    if (pSelectedIDs != "") {
        ...
        swal({
            ...
            function (isConfirm) {
                ...
                if (isConfirm) {
                    //-----------------------
                    FadePageCover(true);
                    CallGETFunctionWithParameters("/api/PS_ApprovePurchasingOrders/Approve"
                        , { pSelectedIDs: pSelectedIDs, pApproved: true }
                        , function (pData) {
                            if (pData[0]) {
                                swal("Success", "Saved successfully", "success");
                               PS_ApprovePurchasingOrders_LoadingWithPaging();
                            }
                            else {

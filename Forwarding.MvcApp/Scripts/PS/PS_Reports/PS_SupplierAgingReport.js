...
function ClientAging_Print(pOutputTo) {
    debugger;
    var pSubAccountIDList = GetAllSelectedIDsAsStringWithNameAttrRPT("nameCbAccount");
  ...
  //  else if (//*$("#cbIsByCostCenter").prop("checked")*// && pMainAccount == "")
    ...
    //    swal("Sorry", "Please, select at least one cost center.");
        ...
        if ($("#cbIsDebit").prop("checked")) {
            ...
            arr_Values.push(pSubAccountIDList);
            var pParametersWithValues =
                {
                    arr_Keys: arr_Keys
                    , arr_Values: arr_Values
                   , pTitle: "Supplier Aging"
                    , pReportName: "Rep_A_SupplierAgingDebit"
                };
            var win = window.open("", "_blank");
            var url = '/ReportMainClass/PrintClientAgingDebit?pTitle="' + pParametersWithValues.pTitle + '"' + '&arr_Keys=' + pParametersWithValues.arr_Keys + '' + '&arr_Values=' + pParametersWithValues.arr_Values + '  ' + '&pReportName=' + pParametersWithValues.pReportName + '';

            win.location = url;
        }
        if ($("#cbIsCredit").prop("checked")) {
            var arr_Keys = new Array();
            var arr_Values = new Array();
            arr_Keys.push("Date");
            arr_Keys.push("SupplierIDs");
            arr_Keys.push("IsPaymentPeriod");
            ...
            var pParametersWithValues =
                {
                    ...
                    , arr_Values: arr_Values
                   , pTitle: "Supplier Aging"
                    , pReportName: "Rep_A_SupplierAgingByDays"
                };
            var win = window.open("", "_blank");
            var url = '/ReportMainClass/PrintClientAgingDebit?pTitle="' + pParametersWithValues.pTitle + '"' + '&arr_Keys=' + pParametersWithValues.arr_Keys + '' + '&arr_Values=' + pParametersWithValues.arr_Values + '  ' + '&pReportName=' + pParametersWithValues.pReportName + '';
...
function Details_FillCheckboxList() {
    ...
    var pAccountRealCode = $("#slAccountsGroup option:selected").attr("RealSubAccountCode");

    CallGETFunctionWithParameters("/api/PS_SupplierAgingReport/LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned"
        , {
       ...
       // , pWhereClause: "4"
        ...
        , pWhereClause: (pAccountRealCode == 0 || pAccountRealCode == null || pAccountRealCode == "" ? "3" : pAccountRealCode)

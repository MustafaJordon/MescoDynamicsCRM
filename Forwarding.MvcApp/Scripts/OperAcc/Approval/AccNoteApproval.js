...
function AccNoteApproval_LoadingWithPaging() {
    debugger;
    ...
    LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", strLoadWithPagingFunctionName, pWhereClause, pOrderBy, pPageNumber, pPageSize, pControllerParameters, function (pData) { AccNoteApproval_BindTableRows(JSON.parse(pData[0])); });
    HighlightText("#tblAccNotes>tbody>tr", $("#txt-Search").val().trim());
}
function AccNoteApproval_GetWhereClause() {
    debugger;
...
function AccNoteApproval_PartnerTypeChanged() {
    //debugger;
    //$("#slPartner").val("");
    ...
    //AccNoteApproval_LoadingWithPaging();
    debugger;
...
function AccNoteApproval_SetApproval(pIsApprove) {
    debugger;
    ...
    if (pSelectedIDs == "" || pSelectedIDs.split(",").length > 1)
        ...
        function () {
            ...
            var pParametersWithValues = {
                ...
                , pWhereClause: AccNoteApproval_GetWhereClause()
            ...
            CallGETFunctionWithParameters("/api/AccNote/ApproveOrUnApprove", pParametersWithValues
                , function (pData) {
                    ...
                    if (pData[0]) {
                        AccNoteApproval_BindTableRows(JSON.parse(pData[1]));
    ...
    if ($("#slEditAccNotePartner").val() == "")
        ...
        if (pSelectedItemsIDs != "") {
            ...
            CallGETFunctionWithParameters("/api/AccNote/AddItems"
                ...
                , function (data) {
                    if (data[0])
                        AccNoteApproval_LoadingWithPaging(); //AccNoteApproval_BindTableRows(JSON.parse(data[1])); //coz the return is different in OperationsEdit(as it retreives Notes on that operations)

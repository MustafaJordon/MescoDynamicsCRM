...
function SC_HideShowEditBtns(IsApproved) {
    ...
    if ($("#hID") == null || $("#hID").val() == "" || $("#hID").val() == "0") { // is [ New ]
        ...
        $('#slCustomers').prop('disabled', false);
    ...
    else // is [ Update ]
    {
        ...
        $('#slCustomers').prop('disabled', true);
...
function SC_SLInvoices_LoadAll(pInvoiceID) {
    debugger;
    if (pInvoiceID == null)
        LoadAll("/api/SC_Transactions/LoadItems", "where 'SlInvoices' = 'SlInvoices' and 40 = 40 and   PartnerRemainedQty*-1 > 0 and  ClientID = " + $('#slCustomers').val() + "", function (pTabelRows) { Fill_SelectInputAfterLoadData_DynamicTypes(pTabelRows, 'ID', 'InvoiceNo', '<-- select sales Invoice -->', '#slSLInvoices', '', false); /*SC_PurchaseItems_BindTableRows(pTabelRows)*/ });
    else
        LoadAll("/api/SC_Transactions/LoadItems", "where 'SlInvoices' = 'SlInvoices' and 40 = 40 and  ClientID = " + $('#slCustomers').val(), function (pTabelRows) { Fill_SelectInputAfterLoadData_DynamicTypes(pTabelRows, 'ID', 'InvoiceNo', '<-- select sales Invoice -->', '#slSLInvoices', pInvoiceID , false); /*SC_PurchaseItems_BindTableRows(pTabelRows)*/ });
...
function SC_PurchaseItems_LoadAll() {
    debugger;
     // ,[TransactionID]
    LoadAll("/api/SC_Transactions/LoadItems", "where 40 = 40 AND (select sum( td.Qty *  td.QtyFactor ) from SC_TransactionsDetails td  join SC_Transactions t on t.ID = td.TransactionID where isnull(td.IsDeleted , 0 ) = 0 and isnull(t.IsDeleted,0) = 0 and(td.TransactionTypeID = 20 or td.TransactionTypeID = 40)  and t.SLInvoiceID = " + $('#slSLInvoices').val() + ") < 0 and ID = " + $('#slSLInvoices').val(), function (pTabelRows) { SC_TransactionsDetails_BindTableRows(pTabelRows) });
    // LoadWithPaging("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/SC_Transactions/LoadWithPaging", $("#div-Pager li.active a").text(), $('#select-page-size').val().trim(), function (pTabelRows) { SC_Transactions_BindTableRows(pTabelRows); SC_Transactions_ClearAllControls(); });
    // HighlightText("#tblSC_Transactions>tbody>tr", $("#txt-Search").val().trim());
}
...
function SC_Transactions_Insert(pSaveandAddNew) {
    ...
    setTimeout(function () {
        ...
        else if ($('#slSLInvoices').val() == "0" || $('#slSLInvoices').val() == "")
        {
            swal('Excuse me', 'Please Select Invoice', 'warning');
            FadePageCover(false)
        }
        else if ($('#slCustomers').val() == "0" || $('#slCustomers').val() == "") {
...
function DrawReport(pTaxes, pDiscount) {
    var TotalNet = 0.00;
    var pReportTitle = "Client Returns Voucher - إذن إرتجاع عميل";
    ...
    ReportHTML += '                 <div class="col-xs-3"><b>Client : </b> ' + $('#slCustomers option:selected').text() + '</div>';
...
function SC_Transactions_FillControls(pID) {
    ...
    setTimeout(function () {
        ...
        SC_SLInvoices_LoadAll($(tr).find("td.SLInvoiceID").attr('val'))


        // $("#slSLInvoices").val($(tr).find("td.SLInvoiceID").attr('val'));
    ...
    $.each(JSON.parse(pItems), function (i, item)
    {
        ...
        AppendRowtoTable("tblItems",
            ("<tr isdeleted='0' ID='" + item.ID + "'  counter='" + (RowsCounter + 1) + "' value='" + item.ID + "'>"
                ...
                + "<td class='Qty' val='" + (typeof item.Qty === 'undefined' ? item.D_PartnerRemainedQty * -1 : item.Qty) + "'>" + "<input onblur='CalculateTotalPrice(this);' tag='" + (typeof item.Qty === 'undefined' ? item.D_PartnerRemainedQty * -1 : item.Qty) + "' type='number' class='inputquantity input-sm  col-sm'>" + (typeof item.Qty === 'undefined' ? ' From <mark>' +  item.D_PartnerRemainedQty * -1 + '</mark>' : '') +   "</td>"
                ...
                + "<td class='InvoicePrice' val='" + (typeof item.InvoicePrice === 'undefined' ? (item.D_Price * item.ExchangeRate) : item.InvoicePrice) + "'>" + (typeof item.InvoicePrice === 'undefined' ? (item.D_UnitPrice * item.ExchangeRate) : item.InvoicePrice) + "</td>"
                ...
                + "<td class='AveragePrice hide' val='" + (typeof item.AveragePrice === 'undefined' ? (item.AveragePrice * item.ExchangeRate) : item.AveragePrice) + "'>" + (typeof item.AveragePrice === 'undefined' ? (item.AveragePrice * item.ExchangeRate) : item.AveragePrice) + "</td>"
                + "<td class='TransactionDate hide' val='" + "0" + "'>" + "0" + "</td>"
                + "<td class='QtyFactor hide' val='" + "1" + "'>" + "1" + "</td>"
                + "<td class='ActualQty hide' val='" + "0" + "'>" + "0" + "</td>"
                + "<td class='TransactionTypeID hide' val='" + "40" + "'>" + "40" + "</td>"
                + "</tr>"));
        RowsCounter++;
...
function AddNewRow() {
    ...
    AppendRowtoTable("tblItems",
        ("<tr ID='" + 0 + "' isdeleted='0'  counter='" + (RowsCounter + 1) + "' value='" + 0 + "'>"
            ...
            + "<td class='QtyFactor hide' val='" + "1" + "'>" + "1" + "</td>"
            + "<td class='ActualQty hide' val='" + "0" + "'>" + "0" + "</td>"
            + "<td class='TransactionTypeID hide' val='" + "40" + "'>" + "0" + "</td>"
            + "</tr>"));

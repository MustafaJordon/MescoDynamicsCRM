...
function SC_Transactions_BindTableRows(pSC_Transactions) {
    ...
    $.each(pSC_Transactions, function (i, item) {
        debugger;
        var ClosedStatueRow = " <td class='bg-warning' style ='font-size:15px;' > <span class='fa fa-unlock'></span> </td>";
        var ClosedButton = " ";
         //" <td class='bg-warning' style ='font-size:15px;' > <span class='fa fa-unlock'></span> </td>";
        if (item.IsClosed == true)
        {
            ClosedStatueRow = " <td class='bg-danger' style ='font-size:15px;' > <span class='fa fa-lock'></span> </td>";
            ClosedButton = "";
        }
        ...
        AppendRowtoTable("tblSC_Transactions",
            ("<tr ID='" + item.ID + "' ondblclick='SC_Transactions_EditByDblClick(" + item.ID + " , " + item.IsApproved + ");'>"
                ...
                + "<td class=' ' val='" + 0 + "'>" + " <a href='#'  onclick='LoadOutgoingReport(" + item.ID + ");' class='btn btn-lightblue btn-sm'  ><i class='fa fa-print'></i>&nbsp; Print outgoing Items &nbsp;</a>" + ClosedButton + "</td>"
...
function SC_Transactions_LoadingWithPaging() {
    ...
    if ($('#txtCustomer_Filter').val().trim() != "")
    {
        WhereClause += " AND PartnerName like N'%" + $('#txtCustomer_Filter').val() + "%'";
    }
...
function SC_Transactions_Insert(pSaveandAddNew) {
    ...
    $($('#tblItems > tbody > tr')).each(function (i, tr) {
        ...
        var fromstore = $('#slFromStore').val();
        var tostore = $('#slToStore').val();
        ...
        console.log(unitid);
        if ($('#txtEntitlementDays').val() == "" || itemid.trim() == "0" || fromstore == "0" || tostore == "0" || quantityid.trim() == "" || quantityid.trim() == "0" || unitid == null || unitid.trim() == "0" || $('#slCustomers').val().trim() == "0") {
    ...
    setTimeout(function () {
        ...
        if ($('#tblItems > tbody > tr').length == 0) {
            ...
            swal('Excuse me', 'Can not duplicate items or Fill All Items , Quantity , Customer , Stores , Units ,Entitlement Days', 'warning');
            FadePageCover(false)
            ...
            InsertUpdateFunctionAndReturnID("form", "/api/SC_Transactions/Insert", {
                ...
                pSLInvoiceID: "0",
                pDepartmentID: "0",
                pClientID: $('#slCustomers').val(),
                pCostCenterID: "0",
                pIsSpareParts: "false",
                pFiscalYearID: "0",
                pSupplierID: "0" ,
                pParentID: "0",
                pTransactionTypeID: "70",
                ...
                pEntitlementDays: $('#txtEntitlementDays').val(),
                pIsClosed: false,
...
function SC_Transactions_Update(pSaveandAddNew) {
    ...
    $($('#tblItems > tbody > tr')).each(function (i, tr) {
        ...
        console.log(unitid);
        if ($('#txtEntitlementDays').val() == "" || itemid.trim() == "0" || fromstore == "0" || tostore == "0" || quantityid.trim() == "" || unitid == null || unitid.trim() == "0" || $('#slCustomers').val().trim() == "0") {
    ...
    setTimeout(function () {
        ...
        if ($('#tblItems > tbody > tr').length == 0) {
            ...
            swal('Excuse me', 'Fill All Items , Quantity , Customer , Stores , Units ,Entitlement Days', 'warning');
            FadePageCover(false)
            ...
            InsertUpdateFunctionAndReturnID("form", "/api/SC_Transactions/Update", {
                ...
                pPurchaseOrderID: "0",
                pExaminationID: "0",
                pIsApproved: (RollBackData.IsApproved == null ? "false" : RollBackData.IsApproved )  ,
                pNotes: ($("#txtNotes").val() == "" || $("#txtNotes").val() == null ? "0" : $("#txtNotes").val()),
                pSLInvoiceID: "0",
                pDepartmentID: "0",
                pClientID: $('#slCustomers').val(),
                ...
                pParentID: "0",
                pTransactionTypeID: "70",
                pJV_ID: (RollBackData.JV_ID == null ? 0 : RollBackData.JV_ID ) , pIsOutOfStore: "true", pMaterialIssueRequesitionsID: 0,
                pToStoreID: $("#slToStore").val()  ,
                pP_ProductionRequestID: 0,
                pP_UnitID: 0,
                pP_ItemID: 0,
                ...
                pP_StartDate: "01/01/1800",
                pEntitlementDays: $('#txtEntitlementDays').val(),
                pIsClosed:( RollBackData.IsClosed == null ? "false" : RollBackData.IsClosed ) ,
                pFromStore: $("#slFromStore").val(),
                pJV_ID2: 0,
                pTransferParentID: 0,
                pForwardingPSInvoiceID: 0,
...
function SC_Transactions_FillControls(pID) {
    ...
    setTimeout(function () {
        ...
        $("#slCustomers").val($(tr).find("td.ClientID").attr('val'));
...
function IntializeData() {
  //  FadePageCover(true);
    ...
    //        Fill_SelectInputAfterLoadData(d[1], 'ID', 'Name', '<-- select supplier -->', '#slCustomers', '');
...
function PrintTransaction() {
    FadePageCover(true)
    var pReportTitle = "Material Issue Request - طــلـب صـــرف"
    ...
    pTablesHTML += '<thead><th>Item</th><th>Unit</th><th>Quantity</th><th>Notes</th></thead>'
    pTablesHTML += '<tbody>';
    $('#tblItems > tbody > tr').each(function (i, tr) {
    ...
    ReportHTML += '                 <div class="col-xs-3"><b>Customer : </b> ' + $('#slCustomers option:selected').text() + '</div>';
    ReportHTML += '                 <div class="col-xs-3"><b>Entitlement Days : </b> ' + $('#txtEntitlementDays').val() + '</div>';
...
function PrintOutGoingReport(data) {
    ...
    ReportHTML += '                 <div class="col-xs-3"><b>Printed On :</b> ' + TodaysDateddMMyyyy + '</div>';
    ReportHTML += '                 <div class="col-xs-3"><b>by :</b> ' + $('#sp-LoginName').html() + '</div>';
    ReportHTML += '                 <div class="col-xs-3"><b>Code : </b> ' + JSON.parse(data)[0].Code + '</div>';
    ReportHTML += '                 <div class="col-xs-3"><b>Customer : </b> ' + JSON.parse(data)[0].PartnerName + '</div>';
    ReportHTML += '                 <div class="col-xs-3"><b>From Store : </b> ' + JSON.parse(data)[0].FromStoreName + '</div>';
    ReportHTML += '                 <div class="col-xs-3"><b>To Store : </b> ' + JSON.parse(data)[0].ToStoreName + '</div>';
    
    //  ReportHTML += '                 <div class="col-xs-3"><b>Purchase Invoice : </b> ' + ($("#slPSInvoices option:selected").val() == "0" ? "-" : $("#slPSInvoices option:selected").text()) + '</div>';
    ReportHTML += '                 <div class="col-xs-3"><b>Transaction Date : </b> ' + GetDateFromServer(JSON.parse(data)[0].TransactionDate ) + '</div>';
    ReportHTML += '                         <div> &nbsp; </div>';
    ReportHTML += pTablesHTML;
    ReportHTML += '         </div>';
...
function LoadOutgoingReport(ID) {
    debugger;
...
function HideShowCloseBtn(sl)
{
    var statue = $(sl).find("option:selected").text();
    //if (statue == "All")
    //{
        $("#btn-Close").addClass("hide");
        $("#btn-Open").addClass("hide");
    //}
    //else if (statue == "Closed")
    //{
    //    $("#btn-Close").addClass("hide");
...
function OpenClose(IsClosed) {
    ...
    if (pSelectedIDs != "") {
        ...
        //        if (isConfirm) {
                    //-----------------------
                    FadePageCover(true);
        CallGETFunctionWithParameters("/api/SC_Transactions/OpenClose"
            , { pSelectedIDs: pSelectedIDs, pIsClosed: IsClosed }
            , function (pData)
            {
                            FadePageCover(false);
                            if (pData[0]) {
    ...
    $.each(JSON.parse(pItems), function (i, item) {
      ...
      //  z
        ...
        AppendRowtoTable("tblItems",
                ...
                + "<td class='TransactionTypeID hide' val='" + "70" + "'>" + "70" + "</td>"
                + "</tr>"));
        RowsCounter++;
    });
    ApplyPermissions();
...
function SetArrayOfItems()
{
    ...
    $("#tblItems>tbody>tr").each(function (i, tr) {
        ...
        objItem.StoreID = $("#slFromStore").val() //$(tr).find('td.StoreID ').find('.selectstore').val();
        ...
        //}
        //else {
        objItem.Notes = ($(tr).find('td.Notes ').find('.inputnotes').val() == "" ? "-" : $(tr).find('td.Notes ').find('.inputnotes').val() );
       // }
...
function AddNewRow() {
    ...
    AppendRowtoTable("tblItems",
        ("<tr ID='" + 0 + "' isdeleted='0'  counter='" + (RowsCounter + 1) + "' value='" + 0 + "'>"
            ...
            + "<td class='QtyFactor hide' val='" + "1" + "'>" + "1" + "</td>"
            + "<td class='ActualQty hide' val='" + "0" + "'>" + "0" + "</td>"
            + "<td class='TransactionTypeID hide' val='" + "70" + "'>" + "0" + "</td>"
            + "</tr>"));

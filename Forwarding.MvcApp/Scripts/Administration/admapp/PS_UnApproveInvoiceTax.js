...
function PS_UnApproveInvoiceTax_BindTableRows(pPS_Invoices) {
    debugger;
    $("#hl-menu-SL").parent().addClass("active");
    ClearAllTableRows("tblPS_UnApproveInvoiceTax");
    $.each(pPS_Invoices, function (i, item) {
        debugger;

        //editControlsText = " class='btn btn-xs btn-rounded btn-info float-right' > <i class='fa fa-pencil' style='padding-left: 5px;'></i> <span style='padding-right: 5px;'>" + TranslateString("Edit") + "</span>";
        AppendRowtoTable("tblPS_UnApproveInvoiceTax",
    ...
    ApplyPermissions();
    BindAllCheckboxonTable("tblPS_UnApproveInvoiceTax", "ID");
    CheckAllCheckbox("ID");
    HighlightText("#tblPS_UnApproveInvoiceTax>tbody>tr", $("#txt-Search").val().trim());
...
function PS_UnApproveInvoiceTax_LoadingWithPaging() {
    debugger;
    ...
    var WhereClause = "Where ISNULL(IsDeleted , 0) = 0 and id in (select originid from ForwardingTransChemTax.dbo.taxlink where originid is not null and notes = 'PS_Invoices' AND JVID IS NOT NULL)";
...
function PS_UnApproveInvoiceTax_Approve() {

    console.log(GetAllSelectedIDsAsString("tblPS_UnApproveInvoiceTax"));
    var pSelectedIDs = GetAllSelectedIDsAsString("tblPS_UnApproveInvoiceTax");
    if (pSelectedIDs != "") {

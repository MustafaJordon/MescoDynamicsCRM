...
function SC_UnApproveTransactionTax_BindTableRows(pSC_Transactions) {
    debugger;
...
function LoadTransactionsDetails(ID) {
    ...
    LoadAll("/api/SC_Transactions/LoadItems", "where 'Details' = 'Details' and ID = " + ID + " ", function (pTabelRows) {
        ...
        SC_TransactionsDetailsTax_BindTableRows(pTabelRows);

    });

}
function SC_TransactionsDetailsTax_BindTableRows(pSC_TransactionsDetails) {
...
function SC_TransactionTax_LoadingWithPaging() {
    debugger;
    ...
    if ($('#slTransactionsTypes').val() == "0") {
        ...
        LoadWithPagingWithWhereClause("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/SC_Approving/LoadWithWhereClause", WhereClause, $("#div-Pager li.active a").text(), $('#select-page-size').val().trim(), function (pTabelRows) { SC_UnApproveTransactionTax_BindTableRows(pTabelRows); });
...
function SC_TransactionsTax_UnApprove() {
                    ...
                    CallGETFunctionWithParameters("/api/SC_Approving/ApproveTax"
                        ...
                        , function (pData) {
                            if (pData[0]) {
                                ...
                                SC_TransactionTax_LoadingWithPaging();
                            }
                            else {
...
function SC_Transactions_DeleteList() {
    ...
    if (GetAllSelectedIDsAsString('tblSC_TransactionsTax') != "")
        swal({
            ...
            function () {
                DeleteListFunction("/api/SC_Transactions/Delete", { "pSC_TransactionsIDs": GetAllSelectedIDsAsString('tblSC_TransactionsTax') }, function () { SC_TransactionTax_LoadingWithPaging(); });
            });
    //DeleteListFunction("/api/SC_Transactions/Delete", { "pSC_TransactionsIDs": GetAllSelectedIDsAsString('tblSC_TransactionsTax') }, function () { SC_Transactions_LoadingWithPaging(); });
}

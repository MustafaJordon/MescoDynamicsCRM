...
function AccountingOperationsPayablesApprovalsTax_BindTableRows(pPayables) {
    debugger;
    ...
    $.each(pPayables, function (i, item) {
        ...
        AppendRowtoTable("tblPayables",
            ...
            ("<tr ID='" + item.ID + "' " + (item.IsApproved ? " class='text-primary' " : "") + ">"
                ...
                + "<td class='IssueDate' val='" + GetDateFromServer(item.IssueDate) + "'>" + GetDateFromServer(item.IssueDate) + "</td>"
                + "<td class='EntryDate' val='" + GetDateFromServer(item.EntryDate) + "'>" + GetDateFromServer(item.EntryDate) + "</td>"
                ...
                //                                                                                        ? ("<a href='#EditPayableModal' data-toggle='modal' onclick='AccountingOperationsPayablesApprovalsTax_SelectCustodyToBeSet(" + item.ID + ");' " + setCustodyControlsText + "</a>")
                //                                                                                        : "")
                //                                                                                : item.CustodyName)
                //+ "</td>"
                ...
                + "<td class=''>"
                //  + "<a href='#'" + (item.IsApproved || item.PartnerSupplierID == 0 ? "disabled='disabled' " : " ") + " onclick='AccountingOperationsPayablesApprovalsTax_SelectCustody(" + item.ID + ",true);' " + editControlsText + "</a>"
                + "<a href='#'" + (item.IsApproved ? "disabled='disabled' " : " ") + " onclick='AccountingOperationsPayablesApprovalsTax_SelectCustody(" + item.ID + ",true);' " + editControlsText + "</a>"
                + "</td>"
...
function AccountingOperationsPayablesApprovalsTax_LoadingWithPaging() {
    debugger;
    ...
    LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", strLoadWithPagingFunctionName, pWhereClause, pOrderBy, pPageNumber, pPageSize, pControllerParameters, function (pData) { AccountingOperationsPayablesApprovalsTax_BindTableRows(JSON.parse(pData[0])); });
    HighlightText("#tblPayables>tbody>tr", $("#txt-Search").val().trim());

}
//called by LoadDataWithPaging
function AccountingOperationsPayablesApprovalsTax_GetWhereClause() {
    debugger;

    $('#lblTotalCost').html(' ');

    //var pWhereClause = "WHERE IsDeleted=0 AND AccNoteID IS NULL AND id in(select originid from ForwardingTransChemTax.dbo.taxlink where originid is NOT null and notes='Payables' AND JVID IS NULL)";
    ...
    if ($("#slApprovalStatus").val() != "") {
        if ($("#slApprovalStatus").val() == "0") {
            if ($("#hDefaultUnEditableCompanyName").val() == "CHM") {
                pWhereClause += " AND ID not IN(select originid from ForwardingTransChemTax.dbo.taxlink where originid is NOT null and notes='Payables' AND JVID IS not NULL)";
            }
            else if ($("#hDefaultUnEditableCompanyName").val() == "OCE") {
                pWhereClause += " AND ID not IN(select originid from ForwardingTROTax.dbo.taxlink where originid is NOT null and notes='Payables' AND JVID IS not NULL)";
            }
        }
        else if ($("#slApprovalStatus").val() == "1") {

            if ($("#hDefaultUnEditableCompanyName").val() == "CHM") {
                pWhereClause += " AND ID IN(select originid from ForwardingTransChemTax.dbo.taxlink where originid is NOT null and notes='Payables' AND JVID IS not NULL)";
            }
            else if ($("#hDefaultUnEditableCompanyName").val() == "OCE") {
                pWhereClause += " AND ID IN(select originid from ForwardingTROTax.dbo.taxlink where originid is NOT null and notes='Payables' AND JVID IS not NULL)";
            }
...
function AccountingOperationsPayablesApprovalsTax_PartnerTypeChanged() {
    debugger;
    //$("#slPartner").val("");
    ...
    //AccountingOperationsPayablesApprovalsTax_LoadingWithPaging();
    debugger;
    ...
    LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned("div-Pager", "select-page-size", "spn-first-page-row", "spn-last-page-row", "spn-total-count", "div-Text-Total", "/api/Payables/AccountingOperationsPayablesApprovalsTax_PartnerTypeChanged", pWhereClause, pOrderBy, pPageNumber, pPageSize, pControllerParameters
        , function (pData) {
...
function AccountingOperationsPayablesApprovalsTax_Approve(pIDs, pIsApprove) {
    debugger;
    var _Suceess = true;
    ...
    var AccountID = "0";
    var AccountIDCharge = "0";

    var SubAccountID = "0";
    var AccountIDCharge = $("#slAccountCharge").val();
    var SubAccountCharge = $("#slSubAccountCharge").val() == null ? "0" : $("#slSubAccountCharge").val();


    var SubAccountChargeCount = $("#slSubAccountCharge").find('option').length;


    if ((SubAccountCharge == "" || SubAccountCharge == "0" || SubAccountCharge == null) && SubAccountChargeCount > 1) {
                            ...
                            swal({
                                ...
                                function () {
                                    ...
                                    var pParametersWithValues = {
                                        ...
                                        , pWhereClause: AccountingOperationsPayablesApprovalsTax_GetWhereClause()
                                        , pPageSize: $("#select-page-size").val()
                                        , pPageNumber: $("#div-Pager li.active a").text() == "" ? 1 : $("#div-Pager li.active a").text()
                                        , pAccountIDCharge: AccountIDCharge
                                        , pSubAccountCharge: SubAccountCharge
                                        , pOrderBy: " OperationID DESC "
                                    };
                                    CallGETFunctionWithParameters("/api/Payables/ApproveOrUnApproveTAX", pParametersWithValues
                                        , function (pData) {
                                            var pStrMessage = pData[2];
                                            if (pData[0]) {
                                                AccountingOperationsPayablesApprovalsTax_BindTableRows(JSON.parse(pData[1]));
...
function Details_FillSlSubAccountCharge(pSlName, pSubAccountID) {
    debugger;
    if ($("#slAccountCharge").val() == 0) //No Account is selected so just empty subaccounts
        $("#slSubAccountCharge").html("<option value=0>" + TranslateString("SubAccount") + "</option>");
    else {
        FadePageCover(true);
        CallGETFunctionWithParameters("/api/ChartOfLinkingAccounts/LoadSubAccountDetails"
            , {
                pLanguage: $("[id$='hf_ChangeLanguage']").val()
                , pWhereClauseSubAccountDetails: "WHERE IsMain=0 AND Account_ID=" + $("#slAccountCharge").val()
                , pOrderBy: "Name"
            }
            , function (pData) {
                FillListFromObject_ERP(pSubAccountID, 4/*pCodeOrName*/, TranslateString("SelectFromMenu"), pSlName, pData[0], null);
...
//function AccountingOperationsPayablesApprovalsTax_SelectCustodyToBeSet(pPayableID) {
//    debugger;
//    jQuery("#SelectCustodyModal").modal("show");
//    $("#btnApplyApprovalAction").removeAttr("onclick");
//    $("#btnApplyApprovalAction").attr("onclick", "AccountingOperationsPayablesApprovalsTax_SetCustody('" + pPayableID + "')");
//}

//function AccountingOperationsPayablesApprovalsTax_SelectCustody(pIDs, pIsApprove) {
//    debugger;
//    if (pIsApprove) {
...
//            if (pSupplierOperationPartnerTypeID == constCustodyOperationPartnerTypeID) //if yes then call AccountingOperationsPayablesApprovalsTax_Approve(pIDs, pIsApprove) directly else offer to set Custody
//                AccountingOperationsPayablesApprovalsTax_Approve(pIDs, pIsApprove);
//            else {
//                jQuery("#SelectCustodyModal").modal("show");
//                $("#btnApplyApprovalAction").removeAttr("onclick");
//                $("#btnApplyApprovalAction").attr("onclick", "AccountingOperationsPayablesApprovalsTax_Approve('" + pIDs + "'," + pIsApprove + ")");
//            }
...
//function AccountingOperationsPayablesApprovalsTax_SetCustody(pPayableID) {
//    debugger;

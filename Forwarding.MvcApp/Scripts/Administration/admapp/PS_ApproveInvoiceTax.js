...
function PS_ApproveInvoiceTax_BindTableRows(pPS_Invoices) {
    debugger;
    $("#hl-menu-SL").parent().addClass("active");
    ClearAllTableRows("tblPS_ApproveInvoiceTax");
    $.each(pPS_Invoices, function (i, item) {
        debugger;

        //editControlsText = " class='btn btn-xs btn-rounded btn-info float-right' > <i class='fa fa-pencil' style='padding-left: 5px;'></i> <span style='padding-right: 5px;'>" + TranslateString("Edit") + "</span>";
        AppendRowtoTable("tblPS_ApproveInvoiceTax",
    ...
    ApplyPermissions();
    BindAllCheckboxonTable("tblPS_ApproveInvoiceTax", "ID");
    CheckAllCheckbox("ID");
    HighlightText("#tblPS_ApproveInvoiceTax>tbody>tr", $("#txt-Search").val().trim());
...
function PS_ApproveInvoiceTax_LoadingWithPaging() {
    debugger;
   // var WhereClause = "Where ISNULL(IsDeleted , 0) = 0 and ISNULL(IsApproved , 0) = 0";
    var WhereClause = "Where ISNULL(IsDeleted , 0) = 0 and  id not in (select originid from ForwardingTransChemTax.dbo.taxlink where originid is not null and notes = 'PS_Invoices' AND JVID IS NOT NULL)";
...
function PS_ApproveInvoiceTax_Approve() {

    console.log(GetAllSelectedIDsAsString("tblPS_ApproveInvoiceTax"));
    var pSelectedIDs = GetAllSelectedIDsAsString("tblPS_ApproveInvoiceTax");
    if (pSelectedIDs != "") {
        ...
        swal({
            ...
            function (isConfirm) {
                ...
                if (isConfirm) {
                    //-----------------------
                    FadePageCover(true);
                    CallGETFunctionWithParameters("/api/PS_Approving/ApproveTax"
                        ...
                        , function (pData) {
                            if (pData[0]) {
                               ...
                               PS_ApproveInvoiceTax_LoadingWithPaging();
                            }
                            else {
                                FadePageCover(false);

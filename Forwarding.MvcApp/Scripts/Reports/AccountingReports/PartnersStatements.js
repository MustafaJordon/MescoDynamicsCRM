        ...
        && ($("#txtToDate").val().trim() == "" || isValidDate($("#txtToDate").val(), 1))
            ...
            pWhereClauseOpenBalance = (" WHERE IsDeleted=0 AND PartnerID = " + $("#slPartner").val() + " AND PartnerTypeID=" + $("#slPartnerType").val()
                    ...
                    + //($("#cbIncludeAllocations").prop("checked")
                        //? 
                        //(" AND TransactionType<>" + constTransactionInvoiceApproval + " AND TransactionType<>" + constTransactionPayableApproval + " AND TransactionType<>" + constTransactionPayableAllocatedFromCustody)  //Make sure not to make conflict between AR/AP Approvals & Inv/Op.Pay Approvals
                        //: 
            ...
            pWhereClauseOpenBalance_RegardingApprovals = (" WHERE IsDeleted=0 AND PartnerID = " + $("#slPartner").val() + " AND PartnerTypeID=" + $("#slPartnerType").val()
                    ...
                    + " OR TransactionType=" + constTransactionPayableAllocatedFromCustody  //Make sure not to make conflict between AR/AP Approvals & Inv/Op.Pay Approvals
                    + " OR TransactionType=" + constTransactionOpenCreditBalance
                    + " OR TransactionType=" + constTransactionOpenDebitBalance

                    + " OR TransactionType=" + constTransactionDebitNote
                    + " OR TransactionType=" + constTransactionCreditNote
                    + ")"  );
            ...
            CallGETFunctionWithParameters("/api/PartnersStatements/LoadData"
...
function Statements_DrawReport(data, pOutputTo) {
    ...
    if (pOutputTo != "Excel") {
        pTablesHTML += '                                 <tr>';
        pTablesHTML += '                                     <td colspan=7><b>Open Balance is ' + pOpenBalance_RegardingApprovals /*+ ' / UnAllocated Open Balance is ' + pOpenBalance*/ + '</b></td>';
        pTablesHTML += '                                 </tr>';
    }

...
//5: Word Document
function TreasuryStatement_Print(pOutputTo) {
    debugger;
    if ($("#slTreasury").val() == "")
        swal("Sorry", "Please, select a treasury.");
    else {
        if (
        ...
        && ($("#txtToDate").val().trim() == "" || isValidDate($("#txtToDate").val(), 1))
            ...
            FadePageCover(true);
            var pWhereClause = TreasuryStatement_GetFilterWhereClause();
            ...
            //    + " AND TransactionType<>" + constTransactionReceivableAllocation
            //    );
            var pWhereClauseOpenBalance = (" WHERE IsDeleted=0 AND TreasuryID = " + $("#slTreasury").val()
                + ($("#slCurrency").val() == "" ? "" : (" AND CurrencyID = " + $("#slCurrency").val()))
                + " AND CreationDate < '" + GetDateWithFormatyyyyMMdd($("#txtFromDate").val().trim()) + "'"
                + ($("#cbIncludeUnApproved").prop("checked") ? "" : " AND IsApproved=1 ")
                + ($("#slPRType").val() == "" ? "" : " AND PRType=" + $("#slPRType").val() + " ")
                );
            var pWhereClauseEndBalance = (" WHERE IsDeleted=0 AND TreasuryID = " + $("#slTreasury").val()
                + ($("#slCurrency").val() == "" ? "" : (" AND CurrencyID = " + $("#slCurrency").val()))
                + " AND CAST(Creationdate AS date) <= '" + GetDateWithFormatyyyyMMdd($("#txtToDate").val().trim()) + "'"
                + ($("#cbIncludeUnApproved").prop("checked") ? "" : " AND IsApproved=1 ")
                + ($("#slPRType").val() == "" ? "" : " AND PRType=" + $("#slPRType").val() + " ")
            ...
            CallGETFunctionWithParameters("/api/TreasuriesStatements/LoadData"
                , pParametersWithValues
                , function (data) {
                    if (data[0]) //pRecordsExist
                        TreasuryStatement_DrawReport(data, pOutputTo);
                    else
...
function TreasuryStatement_GetFilterWhereClause() {
    ...
    pTreasuryFilter = ($("#slTreasury").val() == "" ? "" : " TreasuryID = " + $("#slTreasury").val());
    if (pTreasuryFilter != "" && pWhereClause != "")
        pWhereClause += " AND " + pTreasuryFilter;
    else
        if (pTreasuryFilter != "" && pWhereClause == "")
            pWhereClause += " WHERE " + pTreasuryFilter;
...
function TreasuryStatement_PartnerTypeChanged() {
    debugger;
    $("#slPartner").val("");
...
function TreasuryStatement_DrawReport(data, pOutputTo) {
    debugger;
    ...
    var pReportTitle = "Treasury Statement";
    ...
    if (pOutputTo != "Excel") { //in case od excel, i add it to as the 1st row in the table body
        pTablesHTML += '                                 <tr>';
        pTablesHTML += '                                     <td colspan=7><b>Open Balance is ' + pOpenBalance + '</b></td>';
        pTablesHTML += '                                 </tr>';
    }
    ...
    if (pOutputTo == "Excel") {
        ...
        pTableSummary += '                                     </tr>';
        pTableSummary += '                                     <tr style="font-size:95%;">';
        pTableSummary += '                                         <td>' + ($("#slPRType").val() == "" ? "Available balance in Default Currency:" : "Total in Default Currency:") + '</td>';
        ...
        ExportHtmlTableToCsv_RemovingCommasForNumbers("tblStatements", "Treasury Statement");
    }
    else {
        ...
        ReportHTML += '             <div class="col-xs-5"><b>Treasury :</b> ' + $("#slTreasury option:selected").text() + '</div>';

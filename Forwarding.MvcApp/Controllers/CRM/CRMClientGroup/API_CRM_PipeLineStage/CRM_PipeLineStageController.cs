...
{
    public class CRM_PipeLineStageController : ApiController
    {
        ...
        public Object[] LoadAll(string pWhereClause)
        {
            ...
            CvwCRM_privilegeLog objCvwCRM_privilegeLog = new CvwCRM_privilegeLog();
            objCvwCRM_privilegeLog.GetList(" Where 1=1");
            ...
            return new Object[] { new JavaScriptSerializer().Serialize(objCvwCRM_privilegeLog.lstCVarvwCRM_privilegeLog)
        ...
        public Object[] LoadWithPaging(Int32 pPageNumber, Int32 pPageSize, string pSearchKey)
        {
            ...
            string whereClause = " Where (ClientName LIKE '%" + pSearchKey + "%' "
                ...
                + " OR PipeLineStageName LIKE '%" + pSearchKey + "%' )";
            ...
            objCvwCRM_privilegeLog.GetListPaging(pPageSize, pPageNumber, whereClause, " CreationDate DESC ", out _RowCount);

            return new Object[] { new JavaScriptSerializer().Serialize(objCvwCRM_privilegeLog.lstCVarvwCRM_privilegeLog), _RowCount };
        }
        
        [HttpGet, HttpPost]
        public Object[] CRM_PipeLineStage_LoadData(int pSalesManID ,String pActionType, int pClientID ,int pPipeLineStageID,int pIsManager)
        {
            CvwCRM_privilegeLog objCvwCRM_privilegeLog = new CvwCRM_privilegeLog();
            Int32 _RowCount = 0;// objCvwCRM_Actions.lstCVarCRM_Actions.Count;
                                //ClientID mActionType CreatorUsername PipeLineStageID
            String pWhereClause = " Where 1=1 ";
            if (pSalesManID > 0)
                pWhereClause += " AND CreatorUserID = " + pSalesManID;
            if (pActionType != "All Actions")
                pWhereClause += " AND ActionType = '" + pActionType +"'";
            if (pClientID > 0)
                pWhereClause += " AND ClientID = " + pClientID;
            if (pPipeLineStageID > 0)
                pWhereClause += " AND PipeLineStageID = " + pPipeLineStageID;

            if (pIsManager == 0)
                pWhereClause += " AND CreatorUserID = " + WebSecurity.CurrentUserId;

            objCvwCRM_privilegeLog.GetList(pWhereClause);

            return new Object[] { new JavaScriptSerializer().Serialize(objCvwCRM_privilegeLog.lstCVarvwCRM_privilegeLog), _RowCount };
        }
        
        [HttpGet, HttpPost]
        public Object[] CRM_PipeLineStage_LoadWithPaging(int pPageNumber, int pPageSize, String pOrderBy, int pSalesManID, String pActionType, int pClientID, int pPipeLineStageID, int pIsManager)
        {
            ...
            //objCvwCRM_privilegeLog.GetList(pWhereClause);
            objCvwCRM_privilegeLog.GetListPaging(pPageSize, pPageNumber, pWhereClause, pOrderBy,out _RowCount);
            return new Object[] { new JavaScriptSerializer().Serialize(objCvwCRM_privilegeLog.lstCVarvwCRM_privilegeLog), _RowCount };
        }

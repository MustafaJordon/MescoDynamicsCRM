...
namespace Forwarding.MvcApp.Controllers.CRM.CRMSalesMen
{
    public class CommissionTargetController : ApiController
    {
        ...
        public object[] LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned(bool pIsLoadArrayOfObjects, string pLanguage, Int32 pPageNumber, Int32 pPageSize, string pWhereClause, string pOrderBy)
        {
            ...
            CNoAccessTargetType objCCNoAccessTargetType = new CNoAccessTargetType();
            if (pIsLoadArrayOfObjects)
            {
                checkException = objCSalesman.GetList("WHERE IsSalesman=1 ORDER BY Name");
                checkException = objCCNoAccessTargetType.GetList("ORDER BY ID");
            }
            ...
            return new object[] {
                ...
                , new JavaScriptSerializer().Serialize(objCSalesman.lstCVarUsers) //pSalesman=pData[2]
                , new JavaScriptSerializer().Serialize(objCCNoAccessTargetType.lstCVarNoAccessTargetType) //pTargetType=pData[3]
            };
        }
        ...
        public object[] Save(Int32 pSalesmanID, Int32 pTargetYear, Int32 pTargetTypeID, string pNotes
            ...
            , Int32 pID11, decimal pAmount11, decimal pPercentage11, Int32 pID12, decimal pAmount12, decimal pPercentage12,decimal pGrossProfit)
        {
            string strMessage = "";
            Exception checkException = null;
            ...
            objCVarCRM_CommissionTarget.GrossProfit = pGrossProfit;
        ...
        public object[] LoadSalesReportData(string pLanguage, Int32 pPageNumber, Int32 pPageSize
            ...
            , string pWhereClauseGetFixedTarget, Int32 pTargetType, Int32 pIsBusinessVolume
            , bool pIsGroupByMonth)
        {
            ...
            var pCommissionGroupByMonth = objCvwCRM_SalesReport.lstCVarvwCRM_SalesReport
                .GroupBy(g => new { g.OpenDate.Month, g.Percentage, g.TargetTypeID, g.FixedAmount })
                    .Select(g => new
                    {
                        OperationMonth = g.First().OpenDate.Month
                        ,
                        Receivables = g.Sum(s => s.Receivables)
                        ,
                        Payables = g.Sum(s => s.Payables)
                        ...
                        TargetTypeID = g.First().TargetTypeID
                        ,
                        FixedAmount = g.First().FixedAmount
                    }).OrderBy(o => o.OperationMonth).ToList();


            #region by kelany
            if (pTargetType == 50)
                checkException = objCvwCRM_SalesReport.GetListPaging(pPageSize, pPageNumber, pWhereClauseSalesReport, "OperationID", out _RowCount);
            var lstOperation_Count = objCvwCRM_SalesReport.lstCVarvwCRM_SalesReport.Select(x => new { x.SalesmanName, x.Operation_Count }).GroupBy(x => x.SalesmanName).Select(x => x.FirstOrDefault()).ToList();
            var serializer = new JavaScriptSerializer() { MaxJsonLength = Int32.MaxValue };
            var lstBusinessVolumeProfit = objCvwCRM_SalesReport.lstCVarvwCRM_SalesReport.Select(x => new { x.SalesmanName, x.Receivables, x.Payables }).GroupBy(x => x.SalesmanName).Select(cl => new
            {
                SalesmanName = cl.First().SalesmanName,
                Receivables = cl.Sum(c => c.Receivables).ToString(),
                Payables = cl.Sum(c => c.Payables).ToString()
            }).ToList();

            return new object[] {
                pReportFormat == 10 ? serializer.Serialize(pFixedAmountReport.ToList()) : serializer.Serialize(objCvwCRM_SalesReport.lstCVarvwCRM_SalesReport)
                , lstOperation_Count //pData[1]
                , pIsBusinessVolume == 1 && pTargetType == 20 ? lstBusinessVolumeProfit : null //pData[2]
                , serializer.Serialize(pCommissionGroupByMonth) //pData[3]
            };

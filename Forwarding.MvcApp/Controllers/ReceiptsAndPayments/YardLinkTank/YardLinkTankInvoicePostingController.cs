...
{
    public class YardLinkTankInvoicePostingController : ApiController
    {
        [HttpGet, HttpPost]
        public object[] LoadWithPagingWithWhereClauseAndOrderByWithObjectArrayReturned(bool pIsLoadArrayOfObjects, string pLanguage, Int32 pPageNumber, Int32 pPageSize, string pWhereClause, string pOrderBy)
        {
            ...
            CvwInvoiceHeaderYardLinkTank objCvwSL_InvoiceHeader_SelectNotPosted = new CvwInvoiceHeaderYardLinkTank();
            objCvwSL_InvoiceHeader_SelectNotPosted.GetListPaging(pPageSize, pPageNumber, pWhereClause, pOrderBy, out _RowCount);

            return new object[] {
                new JavaScriptSerializer().Serialize(objCvwSL_InvoiceHeader_SelectNotPosted.lstCVarvwInvoiceHeaderYardLinkTank)
                , _RowCount
        ...
        public object[] Post(string pPostedSelectedIDs, bool pPost)
        {
            ...
            {
                //delete voucher
                objCCustomizedDBCall.ExecuteQuery_DataTable("A_UnApproveInvoiceTankYardDeleteVoucher " + "'," + pPostedSelectedIDs + ",'");


                int NumberOfInvoicesDeleted = pPostedSelectedIDs.Split(',').Length;
                for (int i = 0; i < NumberOfInvoicesDeleted; i++)
                {
                    objCCustomizedDBCall.ExecuteQuery_DataTable("A_UnApproveInvoiceYardTank " + int.Parse(pPostedSelectedIDs.Split(',')[i]));
                }
                    ...
                    if (strMessage == "")
                    {
                        ...
                        if (pPostedSelectedIDs != null)
                        {
                            ...
                            for (int i = 0; i < NumberOfInvoices; i++)
                            {
                                ...
                                CLinkYardLinkTankForwInvoices objCLinkYardForwInvoices2 = new CLinkYardLinkTankForwInvoices();
                                objCLinkYardForwInvoices2.GetList("WHERE YardInvoiceID =" + int.Parse(pPostedSelectedIDs.Split(',')[i]));


                                if (objCLinkYardForwInvoices2.lstCVarLinkYardLinkTankForwInvoices.Count == 0)
                                {
                                    ...
                                    if (objCvw_InvoiceHeaderDetailsYardByInvoiceID.lstCVarvw_InvoiceHeaderDetailsYardByInvoiceID.Count > 0 && strMessage == "")
                                    {
                                        for (int y = 0; y < objCvw_InvoiceHeaderDetailsYardByInvoiceID.lstCVarvw_InvoiceHeaderDetailsYardByInvoiceID.Count; y++)
                                        {
                                            ...
                                            objCVarReceivables.Notes = "Generated with invoice Yard Tank.";
                                            ...
                                            objCVarReceivables.ReceiptDate = DateTime.Now;
                                    ...
                                    {
                                        CVarLinkYardLinkTankForwInvoices objvarCLinkYardForwInvoices = new CVarLinkYardLinkTankForwInvoices();
                                        #region Link
                                        if (strMessage == "" && checkException == null)
                                        {
                                            ...
                                            CLinkYardLinkTankForwInvoices objCLinkYardForwInvoices = new CLinkYardLinkTankForwInvoices();
                                            objCLinkYardForwInvoices.lstCVarLinkYardLinkTankForwInvoices.Add(objvarCLinkYardForwInvoices);
                                            checkException = objCLinkYardForwInvoices.SaveMethod(objCLinkYardForwInvoices.lstCVarLinkYardLinkTankForwInvoices);
                                            if (checkException != null) // an exception is caught in the model
                                            {
                                                strMessage = checkException.Message;
                                                _result = false;
                                        ...
                                        if (checkException == null) // an exception is caught in the model
                                        {
                                            ...
                                            objCCustomizedDBCall.CallStringFunction("update YardTransOcean.dbo.invoiceheader set IsPosted=1, JVID=" + JVID + " WHERE ID = " + int.Parse(pPostedSelectedIDs.Split(',')[i]));
                                            _result = true;
                                        }
                                        else
                                        {
                                ...
                                {
                                    CCallCustomizedSP objCCallCustomizedSP = new CCallCustomizedSP();
                                    checkException = objCCallCustomizedSP.CallCustomizedSP_MultiIDs("ERP_ForwWeb_PostingInvoice", "," + objCLinkYardForwInvoices2.lstCVarLinkYardLinkTankForwInvoices[0].ForwInvoiceID + ",", WebSecurity.CurrentUserId, pPost, 0);
                                    if (checkException == null) // an exception is caught in the model
                                    {
                                        string JVID = "";
                                        JVID = objCCustomizedDBCall.CallStringFunction("SELECT JVID FROM invoices  WHERE ID = " + objCLinkYardForwInvoices2.lstCVarLinkYardLinkTankForwInvoices[0].ForwInvoiceID);
                                        objCCustomizedDBCall.CallStringFunction("update YardTransOcean.dbo.invoiceheader set IsPosted=1, JVID=" + JVID + " WHERE ID = " + int.Parse(pPostedSelectedIDs.Split(',')[i]));
                                        _result = true;
                                    }
                                    else

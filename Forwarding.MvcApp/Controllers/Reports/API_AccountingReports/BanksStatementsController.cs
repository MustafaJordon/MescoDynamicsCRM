...
{
    public class BanksStatementsController : ApiController
    {
        [HttpGet, HttpPost]
        ...
        public object[] LoadData(string pWhereClause, string pWhereClauseOpenBalance, string pWhereClauseEndBalance)
        {
            ...
            CvwAccPaymentDetails objCvwBankAccountOpenBalance = new CvwAccPaymentDetails();
            checkException = objCvwBankAccountOpenBalance.GetList(pWhereClauseOpenBalance);
            CvwAccPaymentDetails objCvwAccBankAccountEndBalance = new CvwAccPaymentDetails();
            checkException = objCvwAccBankAccountEndBalance.GetList(pWhereClauseEndBalance);
            #region OpenBalance
            var pSelectedOpenBalance = objCvwBankAccountOpenBalance.lstCVarvwAccPaymentDetails.GroupBy(g => g.CurrencyID)
                            .Select(g => new
                            {
            ...
            #region EndBalance
            var pSelectedEndBalance = objCvwAccBankAccountEndBalance.lstCVarvwAccPaymentDetails.GroupBy(g => g.CurrencyID)
                                        .Select(g => new
                                        {
                                            //Balance = g.Sum(s => (s.CreditAmount - s.DebitAmount))
                                            //Balance = g.Sum(s => (s.CreditAmount - (s.TransactionType != constTransactionReceivableAllocation ? s.DebitAmount : 0)))
